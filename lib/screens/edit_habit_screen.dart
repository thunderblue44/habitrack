...
class _EditHabitScreenState extends State<EditHabitScreen> {
  ...
  late FrequencyUnit _frequencyUnit;
  ...
  void initState() {
    ...
    _frequencyUnit = widget.habit.frequencyUnit;

    // Initialize reminder settings
    _reminderEnabled = widget.habit.reminderEnabled;
    _reminderTime =
        _parseTimeOfDay(widget.habit.reminderTime) ?? TimeOfDay.now();
    _reminderDays = _parseReminderDays(widget.habit.reminderDays);

    // Initialize appearance
    _selectedColor = widget.habit.color ?? HabitColors.getColorCodes().first;
  ...
  List<bool> _parseReminderDays(String? daysString) {
    final List<bool> days = List.generate(7, (_) => false);

    if (daysString != null && daysString.isNotEmpty) {
      final selectedDays = daysString.split(',').map(int.parse).toList();
      ...
      // If no days specified, default to all days
  ...
  Future<void> _updateHabit() async {
    ...
    try {
      ...
      if (success && mounted) {
        ...
        setState(() {
          _error = 'Failed to update habit. Please try again.';
          _isLoading = false;
        });
      }
    } catch (e) {
